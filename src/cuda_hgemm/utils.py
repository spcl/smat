...
def analyze_sparse_matrix(filename: str, row_block=16, col_block=8):
    ...
    for r in range(0, n_rows, row_block):
        ...
        num_blocks_in_rows.append(num_blocks)
        tot_num_blocks += num_blocks       

    num_blocks_in_rows = np.array(num_blocks_in_rows)
    print(f"Total number of blocks: {tot_num_blocks}, min: {np.min(num_blocks_in_rows)}, max: {np.max(num_blocks_in_rows)}, mean: {np.mean(num_blocks_in_rows)}, std: {np.std(num_blocks_in_rows)}")    
    return num_blocks_in_rows
...
if __name__ == "__main__":
    ...
    # create a pandas dataframe to hold three variants (original, reordered, reordered_transposed_reordered) for each  
    # matrix in the suiteSparse dataset. For each matrix, we will calculate:
    # - total number of blocks
    # - min, max, mean, std of number of blocks per row
    sf = pd.DataFrame(columns=['matrix', 'total_blocks', 'min_blocks', 'max_blocks', 'mean_blocks', 'std_blocks'])

    # store also raw data in a nested dictionary
    results = {} 

    foldername = 'reordered_violin/'
    # iterate over all pairs of matrices: original and reordered using Paolo's method
    orginal_matrices = [f for f in os.listdir(foldername) if f.endswith('.mtx') and "reordered" not in f and "padded" not in f]
    for original in orginal_matrices: 
        ...
        res = analyze_sparse_matrix(f'{foldername}/{original}')
        results[original.split('.')[0]] = {}
        results[original.split('.')[0]]["base"] = res
       # sf = sf.append({'matrix': original, 'total_blocks': np.sum(res), 'min_blocks': np.min(res), 'max_blocks': np.max(res), 'mean_blocks': np.mean(res), 'std_blocks': np.std(res)}, ignore_index=True)
        print("Reordered:")
        # analyze_matrix(f'test/{original.split(".")[0]}_reordered.mtx')
        res = analyze_sparse_matrix(f'{foldername}/{original.split(".")[0]}_reordered.mtx')
        results[original.split('.')[0]]["row"] = res
       # sf = sf.append({'matrix': original.split(".")[0] + "_reordered.mtx", 'total_blocks': np.sum(res), 'min_blocks': np.min(res), 'max_blocks': np.max(res), 'mean_blocks': np.mean(res), 'std_blocks': np.std(res)}, ignore_index=True)

        print("Column Reordered:")
        res = analyze_sparse_matrix(f'{foldername}/{original.split(".")[0]}_reordered_transposed_reordered.mtx')
        results[original.split('.')[0]]["row+col"] = res
        #sf = sf.append({'matrix': original.split(".")[0] + "_reordered_transposed.mtx", 'total_blocks': np.sum(res), 'min_blocks': np.min(res), 'max_blocks': np.max(res), 'mean_blocks': np.mean(res), 'std_blocks': np.std(res)}, ignore_index=True)
    ...
    with open('data.p', 'wb') as fp:
        ...
        # Save the dictionary using numpy.savez
    # generate the violin plot from the raw data in the results dictionary
    # We want a violin plot for each matrix, with three variants: base, row, row+col, so 
    # 27 violin plots in total, 9 groups by three series.
    # Each group will have three violin plots, one for each variant.
    # The x-axis will be the matrix name, the y-axis the number of blocks.
    file_path = 'dictionary.npz'
    ...
    df = df.T
    df = df.reset_index()
    df = df.rename(columns={"index": "matrix"})
    df = pd.melt(df, id_vars=["matrix"], value_vars=["base", "row", "row+col"], var_name="variant", value_name="blocks")

    # create the violin plot
    plt.figure(figsize=(20, 10))
    ax = sns.violinplot(x="matrix", y="blocks", hue="variant", data=df, split=True)
    plt.show()
    ...
    # save dataframe results to xls file
    #sf.to_excel("preprocessing_results.xlsx")
