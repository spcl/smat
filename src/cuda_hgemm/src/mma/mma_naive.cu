...
__global__ void mmaNaiveKernelSparse(half *bcsrValuesA, half *B, half *C, size_t M, size_t N, size_t K, size_t nonzeroBlocks, int* blockInfo, int* relativeBlockIndexMapping) {
    // mmaNaiveKernel
...
void mmaNaiveKernel(half *bcsrValuesA, half *B, half *C, size_t M, size_t N, size_t K, size_t nonzeroBlocks, int* blockInfo, int* relativeBlockIndexMapping) {
    dim3 block(WARP_SIZE);
    dim3 grid(div_ceil(N, MMA_N), div_ceil(M, MMA_M));
    mmaNaiveKernelSparse<<<grid, block>>>(bcsrValuesA, B, C, M, N, K, nonzeroBlocks, blockInfo, relativeBlockIndexMapping);
}
